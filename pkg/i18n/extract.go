...
func Write(basePath string, messages []*i18n.Message) error {
	for _, lang := range Languages {
		filePath := filepath.Join(basePath, lang.String()+".yml")
		...
		if err != nil {
			return fmt.Errorf("failed to encode %s strings: %w", lang, err)
		}

		err = encoder.Close()
		if err != nil {
			return fmt.Errorf("failed to close encoder for %s strings: %w", lang, err)
		}

		err = os.WriteFile(filePath, buff.Bytes(), 0o666)
		if err != nil {
			return fmt.Errorf("failed to write %s strings to file: %w", lang, err)
		}
	}
