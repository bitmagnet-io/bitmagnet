...
func (m *Egress) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.UrlPatterns) > 0 {
		for iNdEx := len(m.UrlPatterns) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.UrlPatterns[iNdEx])
			copy(dAtA[i:], m.UrlPatterns[iNdEx])
			i = encodeVarint(dAtA, i, uint64(len(m.UrlPatterns[iNdEx])))
			i--
			dAtA[i] = 0xa
		}
	}
...
func (m *Egress) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field UrlPatterns", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.UrlPatterns = append(m.UrlPatterns, string(dAtA[iNdEx:postIndex]))
			iNdEx = postIndex
		case 2:
			if wireType == 0 {
				var v Method
				...
				if elementCount != 0 && len(m.Methods) == 0 {
					m.Methods = make([]Method, 0, elementCount)
				}
				for iNdEx < postIndex {
					var v Method
					for shift := uint(0); ; shift += 7 {
