...
func (r *FileRotator) Runner() runner.Runner {
	return func(ctx context.Context, cancel context.CancelCauseFunc) (runner.Shutdowner, error) {
		...
		if err := r.fs.MkdirAll(r.subPath, 0o755); err != nil {
			return nil, fmt.Errorf("%w: %w: %w: %w", Err, ErrStart, ErrCreateDirectory, err)
		}
...
func (r *FileRotator) rotate() error {
	...
	fp, err := r.newFileRotatorFile(now)
	if err != nil {
		return fmt.Errorf("%w: %w: %w", Err, ErrRotate, err)
	}
...
func (r *FileRotator) newFileRotatorFile(now time.Time) (*fileRotatorFile, error) {
	f, err := r.fs.OpenFile(r.newFilePath(now), os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o644)
	if err != nil {
		return nil, fmt.Errorf("%w: %w", ErrCreateFile, err)
	}

	return &fileRotatorFile{
		writer: bufio.NewWriterSize(f, r.bufferSize),
		file:   f,
	}, nil
}
...
func (r *FileRotator) newFilePath(now time.Time) string {
	return filepath.Join(r.subPath, fmt.Sprintf("%s.%s.log", r.baseName, now.Format(timeFormat)))
}

func (r *FileRotator) prune(now time.Time) error {
	files, err := r.fs.ReadDir(r.subPath)
	if err != nil {
	...
	for _, name := range backupFiles[:len(backupFiles)-r.maxBackups] {
		...
		_ = r.fs.Remove(filepath.Join(r.subPath, name))
	}
