...
func newSampleInfoHashesWorker(
	...
	return channel.NewWorker(
		func(ctx context.Context, node ktable.Node) error {
			...
			_ = infoHashTriageAdder.Add(
				ctx,
				slice.Map(res.Samples, func(hash protocol.ID) nodeHasPeersForHash {
			...
			kTable.BatchCommand(
				ktable.PutNode{ID: node.ID(), Addr: node.Addr(), Options: []ktable.NodeOption{
			...
			if len(res.Nodes) > 0 {
				// block on the channel for up to a second trying to add sampled nodes to the
				// discoveredNodes
				...
				_ = discoveredNodesAdder.Add(
					timeoutCtx,
					slice.Map(res.Nodes, func(node client.NodeInfo) ktable.Node {
