import { Component, inject } from "@angular/core";
import { Router } from "@angular/router";
import { Title } from "@angular/platform-browser";
import { ThemeManager } from "../themes/theme-manager.service";
import { VersionComponent } from "../version/version.component";
import { TranslateManager } from "../i18n/translate-manager.service";
import { HealthModule } from "../health/health.module";
import { HealthService } from "../health/health.service";
import { ThemeEmitterComponent } from "../themes/theme-emitter.component";
import { AppModule } from "../app.module";
import { BreakpointsService } from "./breakpoints.service";
import { AuthService } from "../auth/auth.service";

@Component({
  selector: "app-layout",
  template: `
    <ng-container *transloco="let t">
      <mat-toolbar class="toolbar-header">
        @if (breakpoints.sizeAtLeast("Medium")) {
          <h1>
            <a routerLink="torrents">
              <mat-icon svgIcon="magnet"></mat-icon>
              <span class="name">bitmagnet</span>
              <app-version />
            </a>
          </h1>
          <nav>
            <a
              mat-button
              routerLink="torrents"
              #linkTorrents="routerLinkActive"
              routerLinkActive
              [class]="linkTorrents.isActive ? 'active' : ''"
            >
              <mat-icon svgIcon="magnet" />
              {{ t("routes.torrents") }}
            </a>
          </nav>
        } @else {
          <h1>
            <a routerLink="/torrents">
              <mat-icon svgIcon="magnet"></mat-icon>
            </a>
          </h1>
        }
        <span class="spacer"></span>
        @if (authService.self$ | async; as self) {
          @if (self.user) {
            <a
              mat-button
              routerLink="account"
              #linkAccount="routerLinkActive"
              routerLinkActive
              [class]="linkAccount.isActive ? 'active' : ''"
              [matTooltip]="t('routes.account')"
            >
              <mat-icon>person</mat-icon>
              {{ self.user.username }}
            </a>
          } @else {
            <a
              mat-button
              routerLink="login"
              #linkLogin="routerLinkActive"
              routerLinkActive
              [class]="linkLogin.isActive ? 'active' : ''"
            >
              <mat-icon>login</mat-icon>
              Login
            </a>
          }
        }
        <app-health-widget />
        @if (authService.hasRole("admin") | async) {
          <button
            mat-icon-button
            routerLink="admin"
            #linkAdmin="routerLinkActive"
            routerLinkActive
            [class]="linkAdmin.isActive ? 'active' : ''"
            [matTooltip]="t('routes.admin')"
          >
            <mat-icon>settings</mat-icon>
          </button>
        }
        <button
          mat-icon-button
          aria-label="Theme"
          [matMenuTriggerFor]="themesMenu"
          [matTooltip]="t('layout.change_theme')"
        >
          <mat-icon>apparel</mat-icon>
        </button>
        <mat-menu #themesMenu="matMenu" class="layout-header-menu">
          @for (th of themeManager.themes; track th.key) {
            <a
              mat-menu-item
              (click)="themeManager.setTheme(th.key)"
              [class]="th.key === themeManager.selectedTheme ? 'active' : ''"
              ><mat-icon>{{ th.dark ? "dark_mode" : "light_mode" }}</mat-icon
              >{{ th.label }}</a
            >
          }
        </mat-menu>
        <button
          mat-icon-button
          [matMenuTriggerFor]="languagesMenu"
          [matTooltip]="t('layout.translate')"
        >
          <mat-icon>translate</mat-icon>
        </button>
        <mat-menu #languagesMenu="matMenu" class="layout-header-menu">
          @for (l of translateManager.availableLanguages; track l.id) {
            <a
              mat-menu-item
              (click)="translateManager.setLanguage(l.id)"
              [matTooltip]="
                l.id === translateManager.getPreferredLanguage()
                  ? undefined
                  : t('languages.' + l.id)
              "
              matTooltipPosition="right"
              matTooltipShowDelay="500"
              [class]="
                l.id === translateManager.getPreferredLanguage() ? 'active' : ''
              "
              >{{ l.label }}</a
            >
          }
        </mat-menu>
        <button
          mat-icon-button
          class="button-external-links"
          [matTooltip]="t('layout.external_links')"
          [matMenuTriggerFor]="externalLinksMenu"
        >
          <mat-icon svgIcon="external-link" />
        </button>
        <mat-menu #externalLinksMenu="matMenu" class="layout-header-menu">
          <a mat-menu-item href="https://bitmagnet.io" target="_blank"
            >bitmagnet.io</a
          >
          <a
            mat-menu-item
            href="https://discord.gg/6mFNszX8qM"
            target="_blank"
            >{{ t("layout.bitmagnet_on_service", { service: "Discord" }) }}</a
          >
          <a
            mat-menu-item
            href="https://github.com/bitmagnet-io/bitmagnet"
            target="_blank"
            >{{ t("layout.bitmagnet_on_service", { service: "GitHub" }) }}</a
          >
          <a
            mat-menu-item
            href="https://opencollective.com/bitmagnet"
            target="_blank"
            >{{
              t("layout.bitmagnet_on_service", { service: "OpenCollective" })
            }}</a
          >
        </mat-menu>
        @if (!breakpoints.sizeAtLeast("Medium")) {
          <a
            mat-icon-button
            class="button-sponsor"
            href="https://opencollective.com/bitmagnet"
            target="_blank"
            [matTooltip]="t('layout.sponsor')"
            ><mat-icon>favorite</mat-icon></a
          >
        } @else {
          <a
            mat-button
            class="button-sponsor"
            href="https://opencollective.com/bitmagnet"
            target="_blank"
            [matTooltip]="t('layout.support_bitmagnet')"
            ><mat-icon>favorite</mat-icon>{{ t("layout.sponsor") }}</a
          >
        }
      </mat-toolbar>
      <div class="app-content">
        <ng-content></ng-content>
      </div>
      <app-theme-emitter />
    </ng-container>
  `,
  styles: [
    `
      .mat-toolbar.toolbar-header {
        position: sticky;
        top: 0;
        z-index: 10;
        --mat-toolbar-title-text-size: 22px;
        --mat-toolbar-standard-height: 56px;
        --mat-icon-color: #fff;
        padding: 0 20px;
        h1 {
          margin-top: -2px;
          //margin-left: 20px;
          margin-right: 20px;
          a {
            text-decoration: none;
          }
          mat-icon {
            position: relative;
            top: 4px;
          }
          .name {
            margin-left: 10px;
            margin-right: 16px;
          }
          app-version {
            font-size: 13px;
          }
        }
        .mdc-button {
          margin-left: 6px;
          --mdc-text-button-label-text-weight: bold;
          --mat-text-button-horizontal-padding: 12px;
          --mdc-text-button-container-height: 30px;
        }
        --mdc-text-button-label-text-size: 15px;
        nav {
          position: absolute;
          left: 340px;
          top: 0;
          height: 56px;
          padding-top: 14px;
          a {
            margin-right: 10px;
          }
          mat-icon[data-mat-icon-name="magnet"] {
            margin-top: -5px;
            overflow: visible;
          }
        }
      }

      .app-content {
        z-index: 1;
        padding-bottom: 10px;
      }

      .layout-header-menu {
        a.active {
          font-weight: bold;
        }
      }

      app-health-widget {
        margin-left: 10px;
      }

      .button-external-links {
        margin-right: 10px;
      }
    `,
  ],
  standalone: true,
  imports: [AppModule, HealthModule, ThemeEmitterComponent, VersionComponent],
})
export class LayoutComponent {
  themeManager = inject(ThemeManager);
  translateManager = inject(TranslateManager);
  authService = inject(AuthService);
  breakpoints = inject(BreakpointsService);
  title = inject(Title);
  router = inject(Router);
  health = inject(HealthService);
}
